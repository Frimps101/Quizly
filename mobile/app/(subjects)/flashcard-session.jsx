  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      {isLoading ? (
        ...
        <View style={styles.center}>
          <Ionicons name="help-circle-outline" size={64} color={theme.textTer} />
          <Text style={[styles.emptyTitle, { color: theme.textTer }]}>No questions in this topic yet</Text>
        ...
        <View style={styles.sessionContainer}>
          <Text style={[styles.tapHint, { color: theme.textTer }]}>
            {flipped ? "Choose an action below" : "Tap card to reveal answer"}
          </Text>

          <Pressable style={styles.cardWrapper} onPress={flipCard}>
            <Animated.View style={[styles.card, { backgroundColor: theme.surface }, frontStyle]}>
              <View style={[styles.cardTag, { backgroundColor: color + "22" }]}>
                <Text style={[styles.cardTagText, { color }]}>Question</Text>
              </View>
              <Text style={[styles.questionText, { color: theme.text }]}>{current?.question_text}</Text>
              <Ionicons name="finger-print-outline" size={28} color={theme.textTer} style={{ marginTop: 24 }} />
            </Animated.View>

            <Animated.View style={[styles.card, { backgroundColor: theme.surface }, backStyle]}>
              ...
              {current?.explanation ? (
                <View style={[styles.explanation, { backgroundColor: theme.accentLight }]}>
                  <Ionicons name="information-circle-outline" size={14} color={color} style={{ marginRight: 4 }} />
                  <Text style={[styles.explanationText, { color: "#8641f4" }]}>{current.explanation}</Text>
                </View>
              ) : null}
            </Animated.View>
          </Pressable>
          ...
          {flipped && (
            <View style={styles.actions}>
              <TouchableOpacity style={[styles.reviewBtn, { backgroundColor: theme.surface }]} onPress={() => markCard(false)}>
...
const styles = StyleSheet.create({
  ...
  emptyTitle: { fontSize: 17, fontWeight: "600", marginTop: 16, textAlign: "center" },
  sessionContainer: { flex: 1, alignItems: "center", paddingTop: 20, paddingHorizontal: 24 },
  tapHint: { fontSize: 13, marginBottom: 20 },
  cardWrapper: { width: CARD_W, height: 280 },
  card: {
    width: CARD_W, height: 280, borderRadius: 24,
  ...
  questionText: { fontSize: 20, fontWeight: "600", textAlign: "center", lineHeight: 28 },
  answerText: { fontSize: 22, fontWeight: "700", color: "#10B981", textAlign: "center", lineHeight: 30 },
  explanation: { flexDirection: "row", alignItems: "flex-start", marginTop: 16, borderRadius: 10, padding: 10 },
  explanationText: { fontSize: 13, flex: 1, lineHeight: 18 },
  ...
  doneTitle: { fontSize: 26, fontWeight: "800", marginTop: 20 },
  doneSub: { fontSize: 15, marginTop: 4, marginBottom: 28 },
